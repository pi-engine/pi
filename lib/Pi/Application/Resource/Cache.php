...
class Cache extends AbstractResource
{
    ...
    public function renderCache($create = true)
    {
        if (empty($this->renderCache) && $create) {
            $this->renderCache = Pi::service('render');
    ...
    public function checkPage(MvcEvent $e)
    {
        ...
        $cacheKey = 'p' . md5($e->getRequest()->getRequestUri());
    ...
    public function checkAction(MvcEvent $e)
    {
        ...
        $cacheKey = 'a' . md5($e->getRequest()->getRequestUri());
        $renderCache->meta('key', $cacheKey)
                   ->meta('namespace', $e->getRouteMatch()->getParam('module'))
                   ->meta('ttl', $cacheMeta['ttl']);
