...
class Urlizer extends AbstractFilter
{
    ...
    {
        return $this->filter($value, $separator, $excludeDot);
    }
    ...
    public function filter($text, $separator = '-', $excludeDot = false)
    {
        ...
        return self::postProcessText($text, $separator, $excludeDot);
    }
    ...
    private static function postProcessText($text, $separator, $excludeDot = false)
    {
        ...
        $stringToReplace = $excludeDot ? '/\W^[.]/' : '/\W/';
        $text            = preg_replace($stringToReplace, ' ', $text);
