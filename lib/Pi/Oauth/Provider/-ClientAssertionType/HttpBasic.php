...
class HttpBasic implements ClientAssertionTypeInterface, Response\ProviderInterface
{
    protected $response;
    protected $storage;

    public function __construct(Storage\ClientCredentialsInterface $storage)
    {
    ...
    public function validateClientData(array $clientData, $grantTypeIdentifier)
    {
        if ($this->storage->checkClientCredentials($clientData['client_id'], $clientData['client_secret']) === false) {
            $this->response = new Response\Error(400, 'invalid_client', 'The client credentials are invalid');
            return false;
        }

        if (!$this->storage->checkRestrictedGrantType($clientData['client_id'], $grantTypeIdentifier)) {
            $this->response = new Response\Error(400, 'unauthorized_client', 'The grant type is unauthorized for this client_id');
            return false;
        }
    ...
    public function getClientCredentials(Request $request)
    {
        ...
        $this->response = new Response\Error(400, 'invalid_client', 'Client credentials were not found in the headers or body');
        return null;
    }
