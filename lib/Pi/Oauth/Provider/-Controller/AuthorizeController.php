...
class Authorize
{
    ...
    public function __construct(Storage\ClientInterface $clientStorage, array $responseTypes = array(), array $config = array(), Scope $scopeUtil = null)
    {
    ...
    public function handleAuthorizeRequest(Request $request, $is_authorized, $user_id = null)
    {
        ...
        if ($is_authorized === false) {
            $this->response = new Response\Redirect($params['redirect_uri'], 302, 'access_denied', "The user denied access to your application", $params['state']);
            return $this->response;
        }
    ...
    public function validateAuthorizeRequest(Request $request)
    {
        ...
        if (!$client_id = $request->query("client_id")) {
            ...
            $this->response = new Response\Error(400, 'invalid_client', "No client id supplied");
            return false;
        }

        // Get client details
        if (!$clientData = $this->clientStorage->getClientDetails($client_id)) {
            $this->response = new Response\Error(400, 'invalid_client', 'The client id supplied is invalid');
            return false;
        }

        $clientData += array('redirect_uri' => null); // this should be set.  We should create ClientData interface
        if ($clientData === false) {
            $this->response = new Response\Error(400, 'invalid_client', "Client id does not exist");
            return false;
        }
        ...
        if (!($redirect_uri = $request->query('redirect_uri')) && !($redirect_uri = $clientData['redirect_uri'])) {
            $this->response = new Response\Error(400, 'invalid_uri', 'No redirect URI was supplied or stored');
            return false;
        }
        ...
        if (isset($parts['fragment']) && $parts['fragment']) {
            $this->response = new Response\Error(400, 'invalid_uri', 'The redirect URI must not contain a fragment');
            return false;
        }
        ...
        if ($clientData["redirect_uri"] && $redirect_uri && !$this->validateRedirectUri($redirect_uri, $clientData["redirect_uri"])) {
            $this->response = new Response\Error(400, 'redirect_uri_mismatch', 'The redirect URI provided is missing or does not match');
            return false;
        }
        ...
        if (!$response_type || !in_array($response_type, array(static::RESPONSE_TYPE_AUTHORIZATION_CODE, static::RESPONSE_TYPE_ACCESS_TOKEN))) {
            $this->response = new Response\Redirect($redirect_uri, 302, 'invalid_request', 'Invalid or missing response type', $state);
            return false;
        }
        if ($response_type == static::RESPONSE_TYPE_AUTHORIZATION_CODE) {
            if (!isset($this->responseTypes['code'])) {
                $this->response = new Response\Redirect($redirect_uri, 302, 'unsupported_response_type', 'authorization code grant type not supported', $state);
                return false;
            }
            if ($this->responseTypes['code']->enforceRedirect() && !$redirect_uri) {
                $this->response = new Response\Error(400, 'redirect_uri_mismatch', 'The redirect URI is mandatory and was not supplied.');
                return false;
            }
        }

        if ($response_type == static::RESPONSE_TYPE_ACCESS_TOKEN && $this->config['allow_implicit'] === false) {
            $this->response = new Response\Redirect($redirect_uri, 302, 'unsupported_response_type', 'implicit grant type not supported', $state);
            return false;
        }

        // Validate that the requested scope is supported
        if (false === $scope) {
            $this->response = new Response\Redirect($redirect_uri, 302, 'invalid_client', 'This application requires you specify a scope parameter', $state);
            return false;
        }

        if (!is_null($scope) && !$this->scopeUtil->checkScope($scope, $this->scopeUtil->getSupportedScopes())) {
            $this->response = new Response\Redirect($redirect_uri, 302, 'invalid_scope', 'An unsupported scope was requested', $state);
            return false;
        }

        // Validate state parameter exists (if configured to enforce this)
        if ($this->config['enforce_state'] && !$state) {
            $this->response = new Response\Redirect($redirect_uri, 302, 'invalid_request', 'The state parameter is required');
            return false;
        }
