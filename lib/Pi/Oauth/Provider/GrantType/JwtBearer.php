<?php
namespace Pi\Oauth\GrantType;

use Pi\Oauth\Response;
use Pi\Oauth\Storage;
use Pi\Oauth\ResponseType;
use Pi\Oauth\Encryption\Jwt;
use Pi\Oauth\ClientAssertionType\ClientAssertionTypeInterface;
...
*/
class JwtBearer implements GrantTypeInterface, Response\ProviderInterface, ClientAssertionTypeInterface
{
    protected $storage;
    protected $response;
    protected $audience = null;
    protected $jwt = null;
    protected $undecodedJWT = null;
    protected $jwtUtil;

    /**
     * Creates an instance of the JWT bearer grant type.
     *
     * @param JwtBearerInterface $storage
     ...
     * @param Jwt OPTIONAL $jwtUtil
     * The class used to decode, encode and verify JWTs.
     */
    public function __construct(Storage\JwtBearerInterface $storage, $audience, Jwt $jwtUtil = null)
    {
     ...
     * @see GrantTypeInterface::validateRequest()
     */
    public function validateRequest($request)
    {
        if (!$request->request("assertion")) {
            $this->response = new Response\Error(400, 'invalid_request', 'Missing parameters: "assertion" required');
     ...
     * @see GrantTypeInterface::getTokenDataFromRequest()
     */
     ...
     * The JWT parameter.
     */
    protected function getJWTParameter($parameter, $default = null)
    {
     ...
     * @see ClientAssertionTypeInterface::getClientDataFromRequest()
     */
    public function getClientDataFromRequest(Request $request)
    {
        ...
        if (!isset($tokenData['iss'])) {
            ...
            $this->response = new Response\Error(400, 'invalid_grant', "Invalid issuer (iss) provided");
        ...
        if (!$key) {
            $this->response = new Response\Error(400, 'invalid_grant', "Invalid issuer (iss) or subject (sub) provided");
     ...
     * @see ClientAssertionTypeInterface::validateClientData()
     */
     ...
     * @see GrantTypeInterface::validateTokenData()
     */
    public function validateTokenData($tokenData, array $clientData)
    {
        ...
        if (ctype_digit($expiration)) {
            ...
            if ($expiration <= time()) {
                $this->response = new Response\Error(400, 'invalid_grant', "JWT has expired");
            ...
            $this->response = new Response\Error(400, 'invalid_grant', "Expiration (exp) time must be present");

            return false;

        } else {
            $this->response = new Response\Error(400, 'invalid_grant', "Expiration (exp) time must be a unix time stamp");
        ...
        if ($notBefore = $tokenData['nbf']) {
            ...
            if (ctype_digit($notBefore)) {
                ...
                if ($notBefore > time()) {
                    $this->response = new Response\Error(400, 'invalid_grant', "JWT cannot be used before the Not Before (nbf) time");

                    return false;
                }

            } else {
                $this->response = new Response\Error(400, 'invalid_grant', "Not Before (nbf) time must be a unix time stamp");
        ...
        if (!isset($aud) || ($aud != $this->audience)) {
            $this->response = new Response\Error(400, 'invalid_grant', "Invalid audience (aud)");
     ...
     */
    public function createAccessToken(ResponseType\AccessTokenInterface $accessToken, array $clientData, array $tokenData)
    {
     ...
     * @see ProviderInterface::getResponse()
     */
