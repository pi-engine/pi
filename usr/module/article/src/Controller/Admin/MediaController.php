...
class MediaController extends FrontMedia
{
    ...
    public function addAction()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $columns = ['id', 'name', 'title', 'description', 'url'];
    ...
    public function deleteAction()
    {
        ...
        $rowAsset = $this->getModel('asset')->select(['media' => $ids]);
        $medias   = [];
        ...
        $this->getModel('asset_draft')->delete(['media' => $ids]);

        // Removing media stats
        $model = $this->getModel('media_stats');
        $model->delete(['media' => $ids]);
        ...
        $this->getModel('media')->delete(['id' => $ids]);
    ...
    {
        if ($type and
            !in_array($type, ['image', 'doc', 'video', 'zip'])
        ) {
            return [];
        }
