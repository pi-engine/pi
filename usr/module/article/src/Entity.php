...
class Entity
{
    ...
    )
    {
        ...
        $select = $modelVisit->select()
            ...
            ->join(
                ['a' => $modelArticle->getTable()],
                sprintf('%s.article = a.id', $modelVisit->getTable()),
                []
            )
            ->where($where)
            ->group([sprintf('%s.article', $modelVisit->getTable())])
            ->order('total DESC');
    ...
    )
    {
        ...
        $articleIds = $userIds = $authorIds = $categoryIds = [];
        $categories = $authors = $users = $tags = $urls = [];
            ...
            foreach ($resultset as &$row) {
                ...
                */
                ...
                if (empty($columns) || in_array('subject', $columns)) {
                    ...
                    $route      = 'article';
                    $row['url'] = Pi::service('url')->assemble($route, [
                        'module' => $module,
    ...
    public static function getEntity($id)
    {
        ...
        // Get attachments
        $resultsetAsset = Pi::model('asset', $module)->select([
            'article' => $id,
            'type'    => 'attachment',
        ]);
        $mediaIds       = [];
        ...
        */
        ...
        if ($relatedIds) {
            ...
            $where   = ['id' => $relatedIds];
            $columns = ['id', 'subject', 'time_publish'];
     ...
     * @return array
     */
    public static function getSummary($from = 'my', $rules = [])
    {
