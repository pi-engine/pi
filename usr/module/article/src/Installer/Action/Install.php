...
class Install extends BasicInstall
{
    ...
    {
        $events = $this->events;
        $events->attach('install.post', [$this, 'initCategory'], 1);
        $events->attach(
            'install.post',
            [$this, 'initDraftEditPageForm'],
            -90
        );
        $events->attach(
            'install.post',
            [$this, 'initDefaultTopicTemplateScreenshot'],
            -90
        );
        $events->attach(
            'install.post',
        ...
        $events->attach(
            'install.post',
            [$this, 'initClonedBlocksPermission'],
            -120
        );
        $events->attach(
            'install.post',
            [$this, 'initCategoryPermission'],
            -200
        );
    ...
    public function initCategory(Event $e)
    {
        ...
        $parent          = $model->select(['name' => 'root'])->current();
    ...
    public function initCloneBlocks(Event $e)
    {
        ...
        if (file_exists($filename)) {
            ...
            if ($file) {
                ...
                // Get root block ID
                $rowBlock = Pi::model('block_root')->select([
                    'module' => 'article',
                    'name'   => $matches[1],
                ]);
    ...
    public function dressupBlocks(Event $e)
    {
        ...
        if (file_exists($filename)) {
            ...
            if ($file) {
                ...
                // Get block ID
                $rowBlock = Pi::model('block')->select([
                    'module' => $module,
                    'name'   => $blockName,
                ]);
                foreach ($rowBlock as $row) {
